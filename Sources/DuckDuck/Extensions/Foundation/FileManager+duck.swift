...
// MARK: - 静态方法
public extension DDExtension where Base: FileManager {
    /// 判断路径是否存在
    /// - Parameter path: 路径
    /// - Returns: 是否存在
    ...
    static func previousPath(_ fullPath: String) -> String {
        return fullPath.dd.as2NSString.deletingLastPathComponent
    }
    ...
    /// - Parameter path: 路径
    /// - Returns: 是否可删除
    static func isDeletableFile(_ path: String) -> Bool {
    ...
    static func pathExtension(_ path: String) -> String {
        return path.dd.as2NSString.pathExtension
    }
    ...
    static func fileSize(_ path: String) -> UInt64 {
        guard self.fileExists(path) else { return 0 }
    ...
    static func isEqual(path1: String, path2: String) -> Bool {
        guard self.fileExists(path1), self.fileExists(path2) else { return false }
    ...
    /// - Returns: 创建结果
    @discardableResult
    static func createFolder(_ path: String) -> (isOk: Bool, message: String) {
        if self.fileExists(path) { return (true, "文件已存在!") }
        do {
    ...
    /// - Returns: 删除结果
    @discardableResult
    static func removeFolder(_ path: String) -> (isOk: Bool, message: String) {
        guard self.fileExists(path) else { return (true, "文件夹不存在!") }
        do {
    ...
    /// - Returns: 创建结果
    @discardableResult
    static func createFile(_ path: String) -> (isOk: Bool, message: String) {
    ...
    /// - Returns: 删除结果
    @discardableResult
    static func removeFile(_ path: String) -> (isOk: Bool, message: String) {
        guard self.fileExists(path) else { return (true, "文件不存在!") }
    ...
    ///   - address: 文件路径
    /// - Returns: 是否追加成功
    static func appendStringToEnd(_ string: String, to file: Any) -> Bool {
        do {
            var fileURL: URL?
            if let url = file as? URL {
                fileURL = url
            } else if let path = file as? String {
                fileURL = path.dd.as2Url
            }
            ...
            fileHandle.write(content.dd.as2Data!)
    ...
    /// - Returns: 写入结果
    @discardableResult
    static func writeData(_ data: Data?, to path: String) -> (isOk: Bool, message: String) {
        guard self.fileExists(self.previousPath(path)) else { return (false, "文件路径不存在!") }
        guard let data else { return (false, "写入数据不能为空!") }
        guard let url = path.dd.as2Url, path.dd.isValidUrl else { return (false, "写入路径错误!") }
    ...
    static func readFile(_ path: String) -> String? {
        guard self.fileExists(path) else { return nil }
    ...
    /// - Returns: 读取结果
    @discardableResult
    static func readData(from path: String) -> (isOk: Bool, data: Data?, error: String) {
        guard self.fileExists(path),
    ...
    static func copyItem(
        ...
        guard self.fileExists(fromPath) else { return (false, "文件路径不存在!") }

        if !self.fileExists(self.previousPath(toPath)),
           isFile
           ? !self.createFile(self.previousPath(toPath)).isOk
           : !self.createFolder(toPath).isOk
        {
            return (false, "要拷贝到的路径不存在!")
        }

        if isCover, self.fileExists(toPath) {
            do {
    ...
    static func moveItem(
        ...
        guard self.fileExists(fromPath) else { return (false, "要移动的文件不存在!") }

        if !self.fileExists(self.previousPath(toPath)),
           isFile
           ? !self.createFile(toPath).isOk
           : !self.createFolder(toPath).isOk
        {
