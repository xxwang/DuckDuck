...
public extension NSMutableAttributedString {
    /// 转换为不可变字符串
    ///
    /// 将 `NSMutableAttributedString` 转换为不可变的 `NSAttributedString`，可以用于需要 `NSAttributedString` 类型的地方。
    ///
    /// 示例：
    /// ```swift
    /// let mutableAttributedString = NSMutableAttributedString(string: "Hello")
    /// let immutableAttributedString = mutableAttributedString.dd_toNSAttributedString()
    /// ```
    /// - Returns: 返回不可变的 `NSAttributedString` 对象
...
public extension NSMutableAttributedString {
    ...
    /// attributedString.dd_append(newAttributedString)
    /// ```
    ...
    /// attributedString.dd_font(UIFont.systemFont(ofSize: 16), for: NSRange(location: 0, length: 5))
    /// ```
    /// - Parameters:
    ///   - font: 字体
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_wordSpacing(1.5, for: NSRange(location: 0, length: 5))
    /// ```
    /// - Parameters:
    ///   - wordSpacing: 字间距
    ///   - range: 目标范围，默认为整个字符串范围
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_letterSpacing(2) // 设置字母间距为 2
    /// attributedString.dd_letterSpacing(-1, for: NSRange(location: 0, length: 5)) // 设置前5个字符的字母间距为 -1
    /// ```
    @discardableResult
    func dd_letterSpacing(_ letterSpacing: CGFloat, for range: NSRange? = nil) -> Self {
        // 使用默认范围或传入的范围来设置字母间距
        let range = range ?? self.dd_fullNSRange()

        // 调用 dd_addAttributes 方法，将字母间距应用到指定的范围
        return self.dd_addAttributes([.kern: letterSpacing], for: range)
    }
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_lineSpacing(5.0, alignment: .center)
    /// ```
    /// - Parameters:
    ///   - lineSpacing: 行间距
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_foregroundColor(.red)
    /// ```
    /// - Parameters:
    ///   - color: 文字颜色
    ///   - range: 目标范围，默认为整个字符串范围
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_underline(.blue)
    /// ```
    /// - Parameters:
    ///   - color: 下划线颜色
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_strikethrough(.red)
    /// ```
    /// - Parameters:
    ///   - color: 删除线颜色
    ///   - range: 目标范围，默认为整个字符串范围
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_firstLineHeadIndent(10.0)
    /// ```
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_obliqueness(0.2)
    /// ```
    /// - Parameters:
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_lineHeight(20) // 设置行高为 20
    /// attributedString.dd_lineHeight(30, alignment: .center) // 设置行高为 30，并将文本居中对齐
    /// ```
    @discardableResult
    func dd_lineHeight(_ lineHeight: CGFloat, alignment: NSTextAlignment = .left, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dd_fullNSRange()
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World\nThis is a test")
    /// attributedString.dd_paragraphSpacing(10) // 设置段落间距为 10
    /// attributedString.dd_paragraphSpacing(15, for: NSRange(location: 0, length: 12)) // 只对前12个字符设置段落间距
    /// ```
    @discardableResult
    func dd_paragraphSpacing(_ spacing: CGFloat, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dd_fullNSRange()
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_backgroundColor(.yellow) // 设置背景颜色为黄色
    /// attributedString.dd_backgroundColor(.green, for: NSRange(location: 0, length: 5)) // 只为前五个字符设置背景颜色
    /// ```
    @discardableResult
    func dd_backgroundColor(_ color: UIColor, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dd_fullNSRange()
        return self.dd_addAttributes([NSAttributedString.Key.backgroundColor: color], for: range)
    }
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_scale(1.5) // 设置缩放因子为 1.5
    /// attributedString.dd_scale(2.0, for: NSRange(location: 0, length: 5)) // 只为前五个字符设置缩放因子为 2
    /// ```
    @discardableResult
    func dd_scale(_ scale: Float = 1.0, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dd_fullNSRange()
        return self.dd_addAttributes([NSAttributedString.Key.expansion: scale], for: range)
    }
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_textShadow(color: .black, offset: CGSize(width: 2, height: 2), radius: 3) // 设置黑色阴影，偏移量为(2,2)，模糊半径为3
    /// attributedString.dd_textShadow(color: .red, offset: CGSize(width: 0, height: 0), radius: 5, for: NSRange(location: 6, length: 5)) // 只对"World"一词设置阴影
    /// ```
    @discardableResult
    func dd_textShadow(color: UIColor, offset: CGSize, radius: CGFloat, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dd_fullNSRange()
        ...
        return self.dd_addAttributes([NSAttributedString.Key.shadow: shadow], for: range)
    }
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_image("image.png", bounds: CGRect(x: 0, y: 0, width: 50, height: 50))
    /// ```
    /// - Parameters:
    ...
    func dd_image(_ image: String, bounds: CGRect = .zero, index: Int = 0) -> Self {
        ...
        self.insert(imageString, at: index)
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_addAttributes([.foregroundColor: UIColor.red], for: range)
    /// ```
    /// - Parameters:
    ///   - attributes: 要添加的属性字典
    ///   - range: 目标范围，默认为整个字符串范围
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_addAttributes([.foregroundColor: UIColor.red], for: "World")
    /// ```
    /// - Parameters:
    ///   - attributes: 要添加的属性字典
    ///   - text: 目标文本
    ...
    func dd_addAttributes(_ attributes: [NSAttributedString.Key: Any], for text: String) -> Self {
        let ranges = self.dd_subNSRanges(with: [text])
        for range in ranges {
            self.dd_addAttributes(attributes, for: range)
        }
        return self
    }
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_addAttributes([.foregroundColor: UIColor.red], toRangesMatching: "\\bWorld\\b")
    /// ```
    /// - Parameters:
    ///   - attributes: 要添加的属性字典
    ...
    /// - Returns: `Self`，支持链式调用
    @discardableResult
    func dd_addAttributes(_ attributes: [NSAttributedString.Key: Any], toRangesMatching pattern: String, options: NSRegularExpression.Options = []) -> Self {
        guard let regex = try? NSRegularExpression(pattern: pattern, options: options) else { return self }
        let matches = regex.matches(in: self.string, options: [], range: NSRange(0 ..< self.length))
    ...
    /// ```swift
    /// let attributedString = NSMutableAttributedString(string: "Hello World")
    /// attributedString.dd_addAttributes([.foregroundColor: UIColor.red], toOccurrencesOf: "World")
    /// ```
    /// - Parameters:
    ///   - attributes: 要添加的属性字典
    ///   - target: 匹配目标字符串
    /// - Returns: `Self`，支持链式调用
    @discardableResult
    func dd_addAttributes(_ attributes: [NSAttributedString.Key: Any], toOccurrencesOf target: some StringProtocol) -> Self {
